...
import { bezoekPerVerblijf, voegBezoekToe } from './bezoek.js';
...
// Een reis een eigen naam geven.
//
// Er is geen reis-id in het datamodel (valkuil 14) — reizen zijn afgeleid. Een
// naam bewaren doen we daarom op de verblijven zelf, in het veld `tripTitle`
// dat groepeerReizen() al kent en al laat winnen van de afgeleide naam. Een
// lege titel wist hem, en dan komt "jul 2019" vanzelf terug.
//
// Bijeffect dat er al was en dat hier handig uitkomt: twee aaneensluitende
// groepen met verschillende titels worden nooit samengevoegd. Zo trek je een
// vakantie die de automaat aan de vorige plakte alsnog los.
export function hernoemReis(stays, reisId, titel) {
  const lijst = Array.isArray(stays) ? stays : [];
  const reis = groepeerReizen(lijst).find(r => r.id === reisId);
  if (!reis) return lijst;

  const hoortErbij = new Set(reis.stays.map(s => s.id));
  const schoon = typeof titel === 'string' ? titel.trim().slice(0, 80) : '';
  const nu = new Date().toISOString();

  return lijst.map(s => (hoortErbij.has(s.id)
    ? { ...s, tripTitle: schoon || null, updatedAt: nu }
    : s));
}
...
// De verblijven van een reis in het logboek verwerken. Pure functie: geen
// fetch, geen state — zodat het geval waar dit ooit op stukliep te testen valt.
//
// Twee gevallen, en het tweede is de reden dat deze functie bestaat:
//
//   verblijf staat er nog niet  → toevoegen, mét zijn bezoeken
//   verblijf staat er al        → alléén de bezoeken aanvullen
//
// Dat tweede ging eerder mis. Bestaande verblijven werden helemaal
// overgeslagen, en dus verdween ook het net berekende bezoek — terwijl de
// planning er direct daarna aan ging. Op 16 augustus 2026 kostte dat vijftien
// aangevinkte bezoeken: beide campings stonden al in het logboek (met een
// `trip_*`-id uit een eerdere "Huidige reis toevoegen"), dus er werd niets
// bewaard en er verscheen niet eens een melding.
//
// Wat overgeslagen worden wél moest beschermen, blijft staan: van een bestaand
// verblijf raken we **alleen `bezocht`** aan. Cijfer, review, foto's en naam
// zijn van de gebruiker, en die mag een tweede import niet overschrijven.
// `voegBezoekToe()` laat bestaande regels bovendien met rust, dus een
// aangepaste datum of een handmatige `hand_`-regel overleeft dit ook.
//
// Geeft { stays, added, bijgewerkt } terug; is beide nul, dan is er niets
// veranderd en hoeft er niet opgeslagen te worden.
export function verwerkReisInLogboek(stays, candidates, tripConfig, bezoek = {}) {
  const uit = [...(stays || [])];
  const indexById = new Map(uit.map((s, i) => [s?.id, i]));
  let added = 0;
  let bijgewerkt = 0;

  for (const s of candidates || []) {
    const id = tripStayId(s);
    const gevonden = bezoek[s?.id] || [];
    const i = indexById.get(id);

    if (i === undefined) {
      uit.push(stayFromTripStay(s, tripConfig, gevonden));
      indexById.set(id, uit.length - 1);
      added++;
      continue;
    }

    if (!gevonden.length) continue;
    const bestaand = uit[i];
    const samen = voegBezoekToe(bestaand.bezocht, gevonden);
    // Niets nieuws? Dan het verblijf niet aanraken — anders krijgt het een
    // nieuwe updatedAt en botst het gezin met zichzelf zonder aanleiding.
    if (samen.length === (bestaand.bezocht || []).length) continue;
    uit[i] = { ...bestaand, bezocht: samen, updatedAt: new Date().toISOString() };
    bijgewerkt++;
  }

  return { stays: uit, added, bijgewerkt };
}

// Voegt de verblijven van een reis toe aan het logboek en vult bij verblijven
// die er al staan de bezoeken aan. Zie verwerkReisInLogboek() hierboven.
// Geeft { added, skipped, bijgewerkt, stays } terug.
export async function archiveTripStays(tripConfig, updatedBy, planning = null) {
  ...
  if (candidates.length === 0) return { added: 0, skipped: 0, bijgewerkt: 0, stays: null };
  ...
  const { stays: next, added, bijgewerkt } =
    verwerkReisInLogboek(current.stays || [], candidates, tripConfig, bezoek);

  // Opslaan zodra er íéts veranderde. Dit hing eerder alleen aan `added`, en
  // dáárdoor ging een reis waarvan de verblijven al in het logboek stonden
  // stilletjes de prullenbak in.
  if (added === 0 && bijgewerkt === 0) {
    return { added: 0, skipped: candidates.length, bijgewerkt: 0, stays: current.stays || [] };
  }

  await saveStayLog(next, updatedBy);
  return { added, skipped: candidates.length - added, bijgewerkt, stays: next };
}
