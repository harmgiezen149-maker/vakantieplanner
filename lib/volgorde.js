...
// Welke punten vormen de route van een dag?
//
// Bij rijden: verblijf → stops → verblijf, want de rit ernaartoe is net zo goed
// rijden. Bij lopen alleen de stops: naar de stad rijd je, en pas daar begint de
// wandeling. Zou het verblijf er bij lopen in blijven, dan kreeg je een
// wandelroute van twaalf kilometer vanaf de camping getekend.
//
// Aangrenzende identieke punten vallen weg — een activiteit óp het verblijf
// levert anders een etappe van nul meter op.
export function routePunten(stops, { begin = null, eind = null, vervoer = 'rijden' } = {}) {
  const lopen = vervoer === 'lopen';
  const halten = (Array.isArray(stops) ? stops : []).filter(geldig);
  // Geen enkele stop met een locatie? Dan valt er niets te routeren. Een lijn
  // van het ene verblijf naar het andere is een verhuizing, geen dagroute.
  if (halten.length === 0) return [];
  const rij = [
    ...(!lopen && geldig(begin) ? [begin] : []),
    ...halten,
    ...(!lopen && geldig(eind) ? [eind] : []),
  ];
  const schoon = rij.filter((p, i) => {
    if (i === 0) return true;
    const vorige = rij[i - 1];
    return Math.abs(p[0] - vorige[0]) > 0.0001 || Math.abs(p[1] - vorige[1]) > 0.0001;
  });
  // Minder dan twee punten: geen lijn te tekenen en niets te berekenen.
  return schoon.length >= 2 ? schoon : [];
}
