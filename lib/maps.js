...
// `naamHint` is de tekst die vóór de link stond in wat je plakte — de Maps-app
// zet daar de naam en het adres neer. Die ging eerder verloren, terwijl de
// server er precies mee kan doen wat hij nodig heeft als de link zelf geen
// naam prijsgeeft.
export async function apiResolveMaps(url, naamHint = null) {
  const res = await fetch('/api/resolve-maps', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'X-Family-Pin': getPin() },
    body: JSON.stringify({ url, naamHint }),
  });
  if (!res.ok) {
