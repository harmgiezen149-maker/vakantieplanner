...
// Waar de gedeelde plek naartoe moet in het logboek:
//
//   'verblijf'  een nieuw verblijf aanmaken (het formulier voorgevuld openen)
//   'bezoek'    aanhaken bij een verblijf dat er al staat
//
// Ontbreekt de parameter, dan is het 'verblijf'. Dat is met opzet de terugval:
// de links die de deelknop vóór deze functie maakte hebben hem niet, en die
// moeten blijven werken.
export const DOELEN = ['verblijf', 'bezoek'];
export const STANDAARD_DOEL = 'verblijf';

// { naam, coords, label, website, doel } → querystring.
...
export function plekNaarParams({ naam, coords, label, website, doel } = {}) {
  const p = new URLSearchParams();
  if (doel && doel !== STANDAARD_DOEL && DOELEN.includes(doel)) p.set('doel', doel);
...
export function plekUitParams(bron) {
  ...
  const gevraagd = (bron.get('doel') || '').trim();
  const doel = DOELEN.includes(gevraagd) ? gevraagd : STANDAARD_DOEL;

  return { naam, coords: [lat, lng], label, website, doel };
}
