...
import { groepeerReizen } from './stayLog.js';
...
export function maakVerslag(stays) {
  ...
  // Verblijven zonder datum ("los") tellen wel mee als verblijf en voor het
  // gemiddelde, maar niet als reis — je bent er ooit geweest, alleen weten we
  // niet wanneer.
  const echteReizen = reizen.filter(r => !r.los);

  const alleCijfers = lijst.filter(s => typeof s.score === 'number').map(s => s.score);
  const perJaar = {};
  for (const s of lijst) {
    for (const [jaar, n] of Object.entries(nachtenPerJaar(s.startDate, s.endDate))) {
      perJaar[jaar] = (perJaar[jaar] || 0) + n;
    }
  }
  for (const r of echteReizen) {
    // Een reis telt bij het jaar waarin hij eindigt — dat is ook hoe de
    // afgeleide naam ("aug 2026") tot stand komt.
    const jaar = r.endDate?.slice(0, 4);
    if (!jaar) continue;
    perJaar[jaar] = perJaar[jaar] || 0;
  }
  ...
  const jaren = Object.keys(perJaar)
    ...
    .map(jaar => ({
      ...
      reizen: echteReizen.filter(r => r.endDate?.slice(0, 4) === jaar).length,
    }));
