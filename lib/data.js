// Relatieve import (niet '@/lib/…'): dan kan `node --test` deze module
...
// ── Welk verblijf is "nu"? ──────────────────────────────────────────
//
// Op de camping wil je de activiteiten in de buurt zien, niet die van het
// verblijf van volgende week. Daarvoor moet de app weten waar je vandaag zit.
// Er is altijd een antwoord: vóór de reis het eerste verblijf, erna het laatste.
export function huidigVerblijf(stays, vandaagKey) {
  const lijst = (Array.isArray(stays) ? stays : [])
    .filter(s => s?.startDate)
    .sort((a, b) => a.startDate.localeCompare(b.startDate));
  if (lijst.length === 0) return null;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(vandaagKey || '')) return lijst[0];

  const omvat = lijst.filter(s => vandaagKey >= s.startDate && vandaagKey <= (s.endDate || s.startDate));
  // Wisseldag: het verblijf dat vandaag begínt wint — daar ga je heen, en dáár
  // wil je 's avonds weten wat er in de buurt is.
  if (omvat.length) return omvat.find(s => s.startDate === vandaagKey) || omvat[0];

  if (vandaagKey < lijst[0].startDate) return lijst[0];
  return lijst[lijst.length - 1];
}

// Bij welk verblijf hoort een activiteit? Het dichtstbijzijnde, hemelsbreed.
//
// Geen straal die je uit je duim zuigt: met twee verblijven splitst het vanzelf
// op de helft, en met één verblijf hoort alles erbij. Activiteiten zonder
// coördinaten krijgen `null` — die kun je nergens aan koppelen, en dat is
// precies waarom ze in de lijst altijd zichtbaar blijven.
export function verblijfPerActiviteit(activities, stays) {
  const metCoords = (Array.isArray(stays) ? stays : [])
    .filter(s => Array.isArray(s?.coords) && s.coords.length === 2);
  const uit = {};
  for (const a of Array.isArray(activities) ? activities : []) {
    if (!a?.id) continue;
    if (!Array.isArray(a.coords) || a.coords.length !== 2 || metCoords.length === 0) {
      uit[a.id] = null;
      continue;
    }
    let beste = null;
    let besteAfstand = Infinity;
    for (const s of metCoords) {
      const d = hemelsbreed(a.coords, s.coords);
      if (d < besteAfstand) { besteAfstand = d; beste = s; }
    }
    uit[a.id] = beste?.id ?? null;
  }
  return uit;
}

// Datumbereik in Nederlandse notatie, bv. "25 jul — 15 aug 2026".
// Eén datum bekend → alleen die datum. Niets bekend → lege string.
export function formatDateRange(startDate, endDate) {
  ...
  if (!endDate) return one(startDate);
  if (!startDate) return one(endDate);

  const s = parseKey(startDate);
  const e = parseKey(endDate);
  if (startDate === endDate) return one(startDate);
...
// Korte omschrijving van de reisperiode, bv. "25 jul — 15 aug 2026"
export function formatPeriod(config) {
  if (!isTripConfigured(config)) return '';
  return formatDateRange(config.startDate, config.endDate);
}

// Bouwt een Google Maps zoek-URL voor een activiteit
export function getMapsLink(activity) {
  if (!activity?.mapsQuery && !activity?.mapsPlaceId) {
    // Geen zoekterm bekend, maar wel exacte coördinaten (bv. een
    // omgevingssuggestie) → open Google Maps op die precieze plek.
    if (activity?.coords) {
...
export function applyLocationOverride(activity, overrides) {
  ...
  if (ov.note !== undefined) merged.note = ov.note;
  if (ov.important !== undefined) merged.important = ov.important;
  return merged;
}
